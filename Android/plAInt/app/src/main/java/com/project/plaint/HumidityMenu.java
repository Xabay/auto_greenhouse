...
public class HumidityMenu extends AppCompatActivity {
    ...
    Runnable timerTask = new Runnable() {
        ...
        public void run() {
            ...
            ProgressBar waterBar = (ProgressBar) findViewById(R.id.humWaterBar);
            GraphView humGraph = findViewById(R.id.humHumidityGraph);
            ...
            humGraph.getGridLabelRenderer().setLabelFormatter(toTime);


            netLock.lock();
            try {
                LineGraphSeries<DataPoint> ser = new LineGraphSeries<DataPoint>(m.getPlotData("h"));
                ser.setColor(Color.parseColor("#F4E99B"));
                humGraph.addSeries( ser );
            } catch (Exception e) {
                noConnection();
            } finally {
                netLock.unlock();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_humidity_menu);
        ...
        //Create variables for screen objects
        SeekBar humidityBar = (SeekBar) findViewById(R.id.humHumidityBar);
        TextView humidityValue = findViewById(R.id.humHumidityValue);
        Button s_button = (Button) findViewById(R.id.humSaveButton);
        Button c_button = (Button) findViewById(R.id.humCancelButton);


        //Get initial current moisture level, set seekbar
        netLock.lock();
        try {
            humidityBar.setProgress( m.getAttr("h"));
        } catch (Exception e) {
            noConnection();
        } finally {
            netLock.unlock();
        ...
        String value = String.valueOf(" " + humidityBar.getProgress()) + " %";
        ...
        s_button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                SeekBar bar = (SeekBar) findViewById(R.id.humHumidityBar);
                netLock.lock();
                try {
                    m.setAttr("h", bar.getProgress());
                } catch (Exception e) {
                    noConnection();
                } finally {
                    netLock.unlock();
